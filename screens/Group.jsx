...
const Group = ({ navigation }) => {
	...
	const getUserExpense = async () => {
		const { data: userExpense, error } = await supabase
			...
			.select(
				`participant_id, expense_id, status, created_at, amount, pending_from, paid_by, users(username)`
			);
		...
		if (userExpense) {
			...
			const amountOwe = userExpense.reduce((acc, item) => {
				if (
					item.pending_from !== userDetails?.user_id &&
					item.status === 'unsettled' &&
					item.paid_by !== userDetails?.user_id
				) {
					return acc + item.amount;
				} else {
					return acc;
